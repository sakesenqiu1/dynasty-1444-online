...
| 类别 | 内容 |
|---|---|
| 起始格局 | **🧹 清空所有国家**（全部省份变成无主荒地）→ **➕ 新建国家** 从零画一张地图 |
| 省份 | 归属（涂地/吸色/橡皮）、省名、税基/生产/兵源、首府 |
| 国家 | 国名、旗色（36 色盘）、首都、存亡（把领土划光即灭国） |
| 外交 | 开局宗主/属国（附庸或傀儡）、盟友、交战方 |
| 军事 | 各省起始驻军与舰队 |
| 工具 | 画笔国家选择、＋纳入接壤省、**⬆ 导入**本地地图接着改、↩ 撤销（整步回滚）、⬇ 下载、📤 提交 |

「无主荒地」（`owner=0`）是可以通行但谁都不属于的省份，渲染成独立的灰绿色。

### 审核

开始页 → **⚙ 地图审核** → 输入管理员口令 → 列表里每条地图都直接带 `✅ 通过` / `❌ 拒绝` / `👁 预览` / `🗑 删除`，点一下即可，不必先预览。预览会把地图套到背景世界里让你直接看。
